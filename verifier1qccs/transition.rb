...
module Transition
  ...
  def __strong_reduction_inter(cfg, restr_ch)
    ...
    when "condition"
      ...
      label = Label.new("if", "proj1", proc.node.target)
      ...
      label_discard = Label.new("if", "proj0", proc.node.target)
  ...
  public
  def tau_plus(cfg)
    ret = [] # of Configuration
    strongs = __strong_reduction_inter(cfg, [])
    strongs.each { |strong_i|
      if strong_i.label.kind == "tau"
        ret << strong_i.cfg
        ret += tau_plus(strong_i.cfg)
      end
    }
  ...
  public
  def tau_plusplus(cfg)
    ret = [] # of Configuration
    strongs = __strong_reduction_inter(cfg, [])
    strongs.each { |strong_i|
      if strong_i.label.kind == "tau"
        strongs_i_strongs = __strong_reduction_inter(strong_i.cfg, [])
        strongs_i_strongs.each{ |s_j|
          ret += tau_plus(s_j.cfg)
        }
      end
    }
    
    ret
  end # def tau_plusplus
