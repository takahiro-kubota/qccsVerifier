...
class Bisimulation
  ...
  private
  def __weak_reduction(cfg)
    ret = [TransNode.new(Label.new("tau", nil, nil), cfg)] # of TransNode
    
    tau_plus(cfg).each { |cfg_i|
      strong_reduction(cfg_i).each { |trans_i|
        if trans_i.label.kind == "tau"
          # do nothing
        else
      ...
      strongsA.each{ |strongsA_i|
        ...
        if flag_found_B == false and lblA_i.kind != "if"
          ...
          weaksifB.each{ |weakifB_j|
            if lblA_i == weakifB_j[0].label
              same_label_exists_uptotau_B = true
              wcfgB_j_true = weakifB_j[0].cfg
              wcfgB_j_false = weakifB_j[1].cfg
        ...
        elsif flag_found_B == false and lblA_i.kind == "if" and lblA_i.name == "proj0"
          next
        end
