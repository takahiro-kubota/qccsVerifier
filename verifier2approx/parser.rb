  ...
  def parse(lexicon)
    while (element = lexicon.shift)
      ...
      when "equation"
        ...
        @equations[equation_name] = [nil, equation_body]
      when "indis"
        indis_name = lexicon.shift
        indis_wrt = lexicon.shift
        unless @nametable.nats.index(indis_wrt)
          raise "undefined length: " + indis_wrt + "in parsing indis " + indis_name
        end
        indis_body = String.new
        part = lexicon.shift
        until part == "end"
          indis_body += part
          part = lexicon.shift
        end
        @equations[indis_name] = [indis_wrt, indis_body]
      when "configuration"
